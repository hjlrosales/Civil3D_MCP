...
/// <see cref="WorkflowDispatcher"/>, the production <see cref="DrawingHealthWorkflowHandler"/>,
/// a fake session/statistics service and the seven fake domain services. Also exposes a
/// <see cref="ToolCatalog"/> over the health tool assembly for SDK-level tests.
/// </summary>
internal static class HealthHarness
{
        ...
        IStyleService? styles = null)
    {
        ...
        services.AddSingleton<IWorkflowHandler<DrawingHealthWorkflow, DrawingHealthReport>, DrawingHealthWorkflowHandler>();
    ...
    internal static WorkflowContext HealthContext(
        ServiceProvider provider,
    ...
    internal static ToolCatalog CreateCatalog(Container container)
        => new(
            new[] { typeof(DrawingHealthReportTool).Assembly },
