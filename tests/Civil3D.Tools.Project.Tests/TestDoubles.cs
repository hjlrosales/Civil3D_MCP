...
/// <summary>
/// Shared harness doubles for the project-summary tests: a fake session, in-memory domain service
...
internal static class TestDoubles
{
    ...
    /// <summary>Canned sample data. All references resolve; one alignment has no description and
    /// one style is unused, so the workflow test can assert recommendations and complexity.</summary>
    internal static class SampleData
    {
        internal static ActiveDrawing Drawing() => new()
        {
            DrawingName = "ProjectSample.dwg",
            DrawingPath = @"C:\Drawings\ProjectSample.dwg",
            DrawingVersion = "AC1032",
            IsModified = false,
            IsReadOnly = false,
            CurrentLayout = "Model",
            IsModelSpaceActive = true,
            DatabaseFingerprint = "fp-project",
            Civil3DVersion = "25.0",
            OpenDocumentsCount = 1,
            CurrentDocumentName = "ProjectSample.dwg",
            CurrentDocumentPath = @"C:\Drawings\ProjectSample.dwg",
        };
